...
def create_repos_list(projects, gerrit, ssh, git):
    ...
    repos_list = {}
    for p in projects:
        project_url = "https://{}{}/{}.git".format(gerrit, API_PREFIX, p)
        code_url = "https://{}{}/gitweb?p={}.git;hb=HEAD;a=blob;f={{path}}{{anchor}}".format(gerrit, API_PREFIX, p)
        anchor = "#l{line}"

        if ssh and len(ssh) == 2:
            user, port = ssh[0], ssh[1]
            project_url = "ssh://{}@{}:{}/{}.git".format(user, gerrit, port, p)
        if git:
            code_url = "https://{}/{}/tree/{{path}}{{anchor}}".format(git, p)
            anchor = "#n{line}"
