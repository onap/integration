...
for the El Alto release.  Depending on the Use Case you are trying to
deploy, you may need to update the docker image manifest with certain
newer docker image versions than what was shipped with ONAP official El Alto
release.

The ONAP integration repository contains a script that will apply the docker
