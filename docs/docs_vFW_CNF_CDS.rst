...
- K8S plugin definition artifact changed to use VF Module Model Identifiers
    ...
    Previously K8S plugin's definition API used user given values for the definition. SO request has user_directives from where values was taken.
...
- Profile creation API identifications changed same way
    ...
    Same change was done for profile creation API. Note that API endpoints are not changed but only the semantics.
...
- Import this postman collection into postman
    :download: `vFW_CNF_CDS.postman_collection.json`_
- Import this postman environment into postman
    :download: `vFW_CNF_CDS.postman_environment.json`_
