...
sudo sed -i -e '/anonymous_enable=/ s/=.*/=YES/' /etc/vsftpd.conf
...
sudo sed -i -e "/allow_anon_ssl=YES/a\\force_local_data_ssl=NO" /etc/vsftpd.conf
...
sudo sed -i -e "/ssl_ciphers=HIGH/a\\anon_root=/tmp/ftp/rop/" /etc/vsftpd.conf
