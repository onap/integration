...
echo "Creating conductor keyspace in music"
curl -vvvvv --noproxy "*" --request POST http://${MUSIC_IP}:8080/MUSIC/rest/v2/keyspaces/conductor -H "Content-Type: application/json" -H "Authorization=${AUTHORIZATION}" -H "ns=conductor" --data @${WORKSPACE}/test/csit/tests/optf-has/has/data/keyspace.json
