...
Check Cli show microservice
    ...
    ${cli_cmd_output}=    Run Process    ${cli_exec_onap_11} microservice-show -d  --service-name test-service --service-version v1 --host-url http://${MSB_IAG_IP}:80 --long    shell=yes
    Log    ${cli_cmd_output.stdout}
    Should Be Equal As Strings    ${cli_cmd_output.rc}    0

Check Cli delete microservice
    [Documentation]    check delete microservice
    ${cli_cmd_output}=    Run Process    ${cli_exec_onap_11} microservice-delete -d  --service-name test-service --service-version v1 --host-url http://${MSB_IAG_IP}:80 --node-ip 23.14.15.156 --node-port 80 --long    shell=yes
    Log    ${cli_cmd_output.stdout}
    Should Be Equal As Strings    ${cli_cmd_output.rc}    0
