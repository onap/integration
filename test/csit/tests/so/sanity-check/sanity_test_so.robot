...
Get Orchestration Requests invlaidcase not null
    Create Session   refrepo  http://127.0.0.1:8080
    &{headers}=  Create Dictionary    Authorization=Basic SW5mcmFQb3J0YWxDbGllbnQ6cGFzc3dvcmQxJA==    Content-Type=application/json    Accept=application/json
...
Create VnfInstance for invalid input connection
    ...
    ${data}=    Get Binary File     ${CURDIR}${/}data${/}createVnf.json
    &{headers}=  Create Dictionary    Authorization=Basic SW5mcmFQb3J0YWxDbGllbnQ6cGFzc3dvcmQxJA==    Content-Type=application/json    Accept=application/json
    Should Not Be Equal As Strings  ${resp}     104

Delete VnfInstance for invalid input connection invalidcase
    Create Session   refrepo  http://127.0.0.1:8080
    ${data}=    Get Binary File     ${CURDIR}${/}data${/}deleteVnf.json
    &{headers}=  Create Dictionary    Authorization=Basic SW5mcmFQb3J0YWxDbGllbnQ6cGFzc3dvcmQxJA==    Content-Type=application/json    Accept=application/json
    Should Not Be Equal As Strings  ${resp}     200	