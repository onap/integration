...
MSB_IAG_IP="127.0.0.1"
MSB_DISCOVERY_IP="127.0.0.1"
...
# start vfc-nslcm
docker run -d --name vfc-nslcm -e MSB_ADDR=${MSB_IAG_IP}:80 nexus3.onap.org:10001/onap/vfc/nslcm
NSLCM_IP=`get-instance-ip.sh vfc-nslcm`

docker logs -f vfc-nslcm > nslcm.log &
...
echo "=====================<<<<>>>======================="
docker cp vfc-nslcm:/var/log/mysql/error.log  ./
cat error.log
