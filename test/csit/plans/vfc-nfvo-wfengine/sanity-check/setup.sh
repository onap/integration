...
docker run -d --name ${IMAGE} -e MSB_ADDR=${MSB_IAG_IP}:80 ${IMAGE_ACTIVITI_NAME}
WFENGINEACTIVITIR_IP=`get-instance-ip.sh ${IMAGE}`

# Wait for initialization
for i in {1..10}; do
    curl -sS ${WFENGINEACTIVITIR_IP}:8410 && break
    echo sleep $i
    sleep $i
done
...
docker run -d --name ${IMAGE} -e MSB_ADDR=${MSB_IAG_IP}:80 ${IMAGE_MGRSERVICE_NAME}
WFENGINEMGRSERVICE_IP=`get-instance-ip.sh ${IMAGE}`
for i in {1..10}; do
    curl -sS ${WFENGINEMGRSERVICE_IP}:8482 && break
    echo sleep $i
    sleep $i
done

# Pass any variables required by Robot test suites in ROBOT_VARIABLES
ROBOT_VARIABLES="-v MSB_IAG_IP:${MSB_IAG_IP} -v WFENGINEACTIVITIR_IP:${WFENGINEACTIVITIR_IP} -v MSB_IP:${MSB_IAG_IP} -v WFENGINEMGRSERVICE_IP:${WFENGINEMGRSERVICE_IP} -v SCRIPTS:${SCRIPTS}"
