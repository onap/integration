...
sed -i "2i$HOST_IP  portal.api.simpledemo.openecomp.org"   /etc/hosts
...
TIME=0
while [ "$TIME" -lt "$TIME_OUT" ]; do
  response=$(curl --write-out '%{http_code}' --silent --output /dev/null http://localhost:8989/ECOMPPORTAL/login.htm); echo $response
