...
def create_vlm(parameters):
    vlm_create_string = "oclip vlm-create -x {} -u {} -p {} -m {}".format(parameters["vendor-name"],
                                                                          parameters["sdc_creator"],
                                                                          parameters["sdc_password"],
    ...
    entitlement_string = "oclip vlm-entitlement-pool-create -x {} -y {} -e {} -z {} -k {} -g {} -l {} -u {} -p {} -m {}".format(
        parameters["entitlement-pool-name"], vlm_id, vlm_version, parameters["entitlement-description"],
        parameters["vendor-name"],
        parameters["expiry-date"], parameters["start-date"], parameters["sdc_creator"],
        parameters["sdc_password"],
        parameters["sdc_onboarding_url"])
    command_out = (os.popen(entitlement_string)).read()
    entitlement_id = (get_out_helper(command_out))[3]

    key_group_string = "oclip vlm-key-group-create -c {} -e {} -x {} -y {} -u {} -p {} -m {}".format(vlm_id,
                                                                                                     vlm_version,
                                                                                                     parameters[
                                                                                                         "key-group-name"],
                                                                                                     parameters[
    ...
    feature_group_string = "oclip vlm-feature-group-create -x {} -y {} -e {} -z {} -g {} -b {} -c {} -u {} -p {} -m {}".format(
        parameters["feature-grp-name"], vlm_id, vlm_version, parameters["feature-grp-desc"], key_group_id,
        entitlement_id,
        parameters["part-no"], parameters["sdc_creator"], parameters["sdc_password"],
        parameters["sdc_onboarding_url"])
    command_out = (os.popen(feature_group_string)).read()
    feature_group_id = (get_out_helper(command_out))[3]

    agreement_string = "oclip vlm-aggreement-create -x {} -y {} -e {} -z {} -g {} -u {} -p {} -m {}".format(
        parameters["agreement-name"],
        vlm_id, vlm_version, parameters["agreement-desc"], feature_group_id, parameters["sdc_creator"],
        parameters["sdc_password"],
        parameters["sdc_onboarding_url"])
    command_out = (os.popen(agreement_string)).read()
    agreement_id = (get_out_helper(command_out))[3]

    submit_string = "oclip vlm-submit -x {} -y {} -u {} -p {} -m {}".format(vlm_id, vlm_version,
                                                                            parameters["sdc_creator"],
                                                                            parameters["sdc_password"],
                                                                            parameters["sdc_onboarding_url"])
...
def create_vsp(parameters, in_list):
    ...
    for name, value in vnfs.iteritems():
        create_string = "oclip vsp-create -j {} -o {} -e {} -x {} -y {} -i {} -c {} -g {} -u {} -p {} -m {}".format(
            in_list[0],
            parameters["onboarding-method"], parameters["vendor-name"], value.get(
                "vsp-name"),
            value.get("vsp-desc"), in_list[1],
            in_list[2], in_list[3], parameters["sdc_creator"], parameters["sdc_password"],
            parameters["sdc_onboarding_url"])
        ...
        os.system("oclip vsp-add-artifact -x {} -y {} -z {} -u {} -p {} -m {}".format(vsp_id, vsp_version,
                                                                                      value.get(
                                                                                          "path"),
                                                                                      parameters[
                                                                                          "sdc_creator"],
        ...
        os.system("oclip vsp-validate -x {} -y {} -u {} -p {} -m {}".format(vsp_id, vsp_version,
                                                                            parameters["sdc_creator"],
                                                                            parameters["sdc_password"],
                                                                            parameters["sdc_onboarding_url"]))

        os.system("oclip vsp-submit -x {} -y {} -u {} -p {} -m {}".format(vsp_id, vsp_version,
                                                                          parameters["sdc_creator"],
                                                                          parameters["sdc_password"],
                                                                          parameters["sdc_onboarding_url"]))

        os.system("oclip vsp-package -x {} -y {} -u {} -p {} -m {}".format(vsp_id, vsp_version,
                                                                           parameters["sdc_creator"],
                                                                           parameters["sdc_password"],
                                                                           parameters["sdc_onboarding_url"]))
...
def create_vf_model(parameters, vsp_dict):
    ...
    vf_dict = {}
    for name, value in vnfs.iteritems():
        create_string = "oclip vf-model-create -y {} -g {} -x {} -z {} -b {} -u {} -p {} -m {}".format(
            value.get("vf-description"),
            value.get(
                "vsp-version"), value.get("vf-name"), parameters["vendor-name"], vsp_dict[name],
            parameters["sdc_creator"], parameters["sdc_password"], parameters["sdc_catalog_url"])
        os.system(create_string)

        output = (
            os.popen("oclip vf-model-list -m {} -u {} -p {} | grep {}".format(parameters["sdc_catalog_url"],
                                                                              parameters["sdc_creator"],
                                                                              parameters["sdc_password"],
                                                                              value.get("vf-name")))).read()
        output = (get_out_helper_2(output))

        vf_unique_id = output[1]

        os.system("oclip vf-model-certify -b {} -r {} -u {} -p {} -m {}".format(vf_unique_id,
                                                                                value.get(
                                                                                    "vf-remarks"),
                                                                                parameters["sdc_creator"],
                                                                                parameters["sdc_password"],
...
def create_service_model(parameters, vf_dict):
    ...
    create_string = "oclip service-model-create -z {} -y {} -e {} -x {} -c {} -b {} -u {} -p {} -m {} |grep ID".format(
        parameters["project-code"],
        parameters["service-model-desc"], parameters["icon-id"], parameters["service-model-name"],
        parameters["category-display"],
        parameters["category"], parameters["sdc_creator"], parameters["sdc_password"],
        parameters["sdc_catalog_url"])

    service_model_id = (get_out_helper_2((os.popen(create_string)).read()))[1]

    for name, value in vnfs.iteritems():
        os.system(
            "oclip service-model-add-vf -x {} -b {} -y {} -z {} -u {} -p {} -m {}".format(service_model_id,
                                                                                          parameters[
                                                                                              "vf-version"],
                                                                                          vf_dict[name],
                                                                                          value.get(
    ...
    os.system("oclip service-model-test-request -b {} -r {} -u {} -p {} -m {}".format(service_model_id,
                                                                                      parameters[
                                                                                          "service-test-remarks"],
                                                                                      parameters[
                                                                                          "sdc_creator"],
                                                                                      parameters[
                                                                                          "sdc_password"],
    ...
    os.system("oclip service-model-test-start -b {} -u {} -p {} -m {}".format(service_model_id,
                                                                              parameters["sdc_tester"],
                                                                              parameters["sdc_password"],
                                                                              parameters["sdc_catalog_url"]))

    os.system("oclip service-model-test-accept -b {} -r {} -u {} -p {} -m {}".format(service_model_id,
                                                                                     parameters[
                                                                                         "service-accept-remarks"],
                                                                                     parameters["sdc_tester"],
                                                                                     parameters[
                                                                                         "sdc_password"],
                                                                                     parameters[
    ...
    service_model_values = (
        os.popen("oclip service-model-list -u {} -p {} -m {} |grep {}".format(parameters["sdc_creator"],
                                                                              parameters["sdc_password"],
                                                                              parameters["sdc_catalog_url"],
                                                                              parameters[
            "service-model-name"]))).read()
    service_model_values = get_out_helper_2(service_model_values)
    service_model_uniqueId = (service_model_values)[1]

    os.system("oclip service-model-approve -b {} -r {} -u {} -p {} -m {}".format(service_model_uniqueId,
                                                                                 parameters[
                                                                                     "service-approve-remarks"],
                                                                                 parameters["sdc_governor"],
                                                                                 parameters["sdc_password"],
                                                                                 parameters[
                                                                                     "sdc_catalog_url"]))

    os.system("oclip service-model-distribute -b {} -u {} -p {} -m {}".format(service_model_uniqueId,
                                                                              parameters["sdc_operator"],
                                                                              parameters["sdc_password"],
                                                                              parameters["sdc_catalog_url"]))
...
def register_cloud_helper(cloud_region, values, parameters):
    ...
    -x {} -y {} -j {} -w {} -l {} -url {} -n {} -q {} -r {} -Q {} -i {} -g {} -z {} -k {} -c {} -m {} -u {} -p {}'.format(
        values.get("esr-system-info-id"), values.get(
            "user-name"), cloud_region, parameters["cloud-owner"],
        cloud_region, values.get("password"), values.get(
            "cloud-region-version"),
        values.get("default-tenant"),
        values.get(
            "service-url"), parameters["complex_name"], values.get("cloud-type"),
        parameters["owner-defined-type"],
        values.get(
            "system-type"), values.get("identity-url"), parameters["cloud-zone"],
        values.get("ssl-insecure"),
        values.get(
            "system-status"), values.get("cloud-domain"), parameters["aai_url"],
        parameters["aai_username"],
        parameters["aai_password"])
    ...
    complex_associate_string = "oclip complex-associate -x {} -y {} -z {} -m {} -u {} -p {}".format(
        ...
        cloud_region, parameters["cloud-owner"], parameters["aai_url"], parameters["aai_username"],
        parameters["aai_password"])
...
def add_customer_subscription(parameters):
    ...
    for cloud_region, cloud_region_values in (
            parameters["cloud_region_data"]).iteritems():
        if subscription_check == 0:
            subscription_string = "oclip subscription-create -x {} -c {} -z {} -e {} -y {} -r {} -m {} -u {} -p {}".format(
                parameters["customer_name"], cloud_region_values.get(
                    "tenant-id"), parameters["cloud-owner"],
                parameters["service_name"],
                cloud_region_values.get(
                    "default-tenant"), cloud_region, parameters["aai_url"],
                parameters["aai_username"], parameters["aai_password"])
        else:
...
def register_vnfm_helper(vnfm_key, values, parameters):
    ...
    -m {} -u {} -p {}'.format(vnfm_key, values.get("type"), values.get("vendor"),
                              values.get("version"), values.get(
                                  "url"), values.get("vim-id"),
                              values.get(
                                  "user-name"), values.get("user-password"),
                              values.get("vnfm-version"),
...
def add_policy_models(parameters):
    ...
    hpa_val = ('hpaPolicy', 'HPA Tests Model', '[]', 'demo',
               'identity=string:defaultValue-null:required-true:MANY-false:description-null',
    ...
    sql = "INSERT INTO microservicemodels (modelname, description, dependency, imported_by, \
          ...
          dataOrderInfo) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val = [
        ('distancePolicy', 'distancePolicy', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         '""'),
        ('optimizationPolicy', 'Optimization policy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         '{"parameterAttributes_properties":{"customerLocationInfo":"string:defaultValue-null:required-true:MANY-false:description-null",\
		   ...
		   "operator":"OPERATOR:defaultValue-null:required-true:MANY-false:description-null"}}',
         'test1', 'policyScope=matching-true, policyType=matching-true',
         'OPERATOR=[*,+,-,/,%,], POLICYTYPE=[placement_optimization,]', '""'),
        ('queryPolicy', 'Query policy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         '""'),
        ('vnfPolicy', 'VnfPolicy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         '""'),
        ('vim_fit', 'Capacity policy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         'test1', 'policyScope=matching-true, policyType=matching-true ',
         ' APPLICABLERESOURCES=[any,all,], POLICYTYPE=[vim_fit,]', '""'),
        ('affinityPolicy', 'Affinity policy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
         ...
         'test1', 'policyScope=matching-true, policyType=matching-true ',
         ' APPLICABLERESOURCES=[any,all,], POLICYTYPE=[zone,], QUALIFIER=[same,different,]', '""'),
        ('pciPolicy', 'Pci policy model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
        ...
        ('subscriberPolicy', 'Subscriber Policy Model', '[]', 'demo',
         'identity=string:defaultValue-null:required-true:MANY-false:description-null',
...
def add_policies(parameters):
    ...
    for policy in os.listdir(parameters["policy_directory"]):
        policy_name = "{}.{}".format(
            parameters["policy_scope"],
            os.path.splitext(policy)[0])
        policy_file = (os.path.join(parameters["policy_directory"], policy))
        # Create policy
        os.system(
            "oclip policy-create-outdated -m {} -u {} -p {} -x {} -S {} -T {} -o {} -b $(cat {})".format(
                parameters["policy_url"],
                parameters["policy_username"], parameters["policy_password"], policy_name,
                parameters["policy_scope"],
                parameters["policy_config_type"], parameters["policy_onapName"], policy_file))

        # Push policy
        os.system(
            "oclip policy-push-outdated -m {} -u {} -p {} -x {} -b {} -c {}".format(parameters["policy_url"],
                                                                                    parameters[
                "policy_username"],
                parameters[
                "policy_password"],
...
def onboard_vnf(parameters):
    ...
    for key, value in vnfs.items():
        vnf_onboard_string = 'oclip vfc-catalog-onboard-vnf -c {}'.format(
            value.get("csar-id"))
...
def onboard_ns(parameters):
    ns_onboard_string = 'oclip vfc-catalog-onboard-ns -c {}'.format(
...
def upload_ns_package(parameters, ns_package_output):
    ns = parameters["ns"]
    ns_upload_string = '{}/api/nsd/v1/ns_descriptors/{}/nsd_content'.format(parameters["vfc-url"],
                                                                            ns_package_output)
    print ns_upload_string
    print ns.get("path")
    resp = requests.put(
        ns_upload_string, files={
            'file': open(
                ns.get("path"), 'rb')})
    return resp
...
def upload_vnf_package(parameters, vnf_package_output):
    ...
    for vnf_key, vnf_values in vnfs.iteritems():
        vnf_upload_str = '{}/api/vnfpkgm/v1/vnf_packages/{}/package_content'.format(parameters["vfc-url"],
            ...
            vnf_key])
        resp = requests.put(
            vnf_upload_str, files={
                'file': open(
                    vnf_values.get("path"), 'rb')})
    return resp
