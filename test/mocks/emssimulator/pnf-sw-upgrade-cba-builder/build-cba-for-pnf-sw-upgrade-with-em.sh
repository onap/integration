...
if [ -d ${TARGET_CBA} ]; then
    BACKUP_DIR="bak-${TARGET_CBA}-`date +%FT%H%M%S`-${RANDOM}"
...
if [ -f ${TARGET_CBA}.zip ]; then
    BACKUP_FILE="bak-${TARGET_CBA}-`date +%FT%H%M%S`-${RANDOM}.zip"
    mv ${TARGET_CBA}.zip ${BACKUP_FILE}
    echo "Backup ${TARGET_CBA}.zip to ${BACKUP_FILE}"
fi

mv ${TARGET_CBA}/${TARGET_CBA}.zip .
