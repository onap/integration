...
def generatexml(jobId,timestemp):
        ...
        tree = ET.parse(pm_location+"template.xml")
        root = tree.getroot()
        attrib = {}
        ...
        value.text = "400"
        tree.write(pm_location+"template.xml",encoding="utf-8", xml_declaration=True)
        shutil.copy(pm_location+"template.xml",pm_location+"A{}.xml".format(timestemp))
