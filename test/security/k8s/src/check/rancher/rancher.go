...
func (r *Rancher) GetAPIParams() ([]string, error) {
	...
	for _, host := range hosts {
		cmd, err := getPsCmdOutput(host)
		if err != nil {
			return []string{}, err
		}

		if len(cmd) > 0 {
			i := bytes.Index(cmd, []byte(check.APIProcess.String()))
			if i == -1 {
				return []string{}, fmt.Errorf("missing %s command", check.APIProcess)
			}
			return btos(cmd[i+len(check.APIProcess.String()):]), nil
		}
	}
	return []string{}, nil
}
...
// getPsCmdOutput returns running Kubernetes API server command with its parameters.
// It queries default environment set in configuration file.
func getPsCmdOutput(host string) ([]byte, error) {
