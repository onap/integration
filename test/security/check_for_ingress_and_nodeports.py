...
#   It require k8s cluster config file on local machine.
#
# Example usage:
#   Display exposed nodeport and ingress resources declared in the K8S cluster without scanning:
#       check_for_ingress_and_nodeports.py --conf ~/.kube/config.onap
#   Scan declared nodeports:
#       check_for_ingress_and_nodeports.py --conf ~/.kube/config.onap --scan-nodeport
#   Scan declared exposed ingress resources:
#       check_for_ingress_and_nodeports.py --conf ~/.kube/config.onap --scan-ingress
...
def console_visualisation(cname, name, retstatus, httpcode, out, mode, httpcodes = []):
...
def check_onap_ingress():
    ...
    for k,v in check_list.items():
        for host in v[0]:
            console_visualisation(k,*scan_port(host,ihttp,ihttps,v[1]))
