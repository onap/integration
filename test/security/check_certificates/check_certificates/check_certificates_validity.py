...
for service in k8s_services:
    try:
        ...
        print("Bad certificate for port %s" % port)
        node_ports_ssl_error_list.append(
            {'pod_name': port.name,
            'pod_node': port.node_port})
    except ConnectionRefusedError:
        print("ConnectionrefusedError for port %s" % port)
        node_ports_connection_error_list.append(
            {'pod_name': port.name,
            'pod_node': port.node_port})
    except TypeError:
        print("Type Error for port %s" % port)
        node_ports_type_error_list.append(
            {'pod_name': port.name,
            'pod_node': port.node_port})
    except ConnectionResetError:
        print("ConnectionResetError for port %s" % port)
